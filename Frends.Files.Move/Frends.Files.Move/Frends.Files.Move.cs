...
public static class Files
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var fileTransferEntries = Helpers.Helpers.GetFileTransferEntries(results.Files, input.SourceDirectory,
            input.TargetDirectory, options.PreserveDirectoryStructure);
        if (options.IfTargetFileExists == FileExistsAction.Throw)
            ImpersonatedAction.Execute(() => Helpers.Helpers.AssertNoTargetFileConflicts(fileTransferEntries.Values.ToArray()),
                ...
                switch (options.IfTargetFileExists)
                {
                    case FileExistsAction.Rename:
                        validName = ImpersonatedAction.Execute(
                            () => Helpers.Helpers.GetNonConflictingTargetFilePath(sourceFilePath, targetFilePath),
                            connection, ImpersonatedPart.Target);
                        await Helpers.Helpers.CopyFileImpersonated(sourceFilePath, validName, connection,
                            cancellationToken);
                        break;
        ...
        catch (Exception)
        {
            ...
            ImpersonatedAction.Execute(
                () => Helpers.Helpers.DeleteExistingFiles(fileResults.Select(x => x.TargetPath)),
                connection, ImpersonatedPart.Target);
            throw;
        }

        ImpersonatedAction.Execute(
            () => Helpers.Helpers.DeleteExistingFiles(fileResults.Select(x => x.SourcePath)),
            connection, ImpersonatedPart.Source);
        return fileResults;
    }
}
