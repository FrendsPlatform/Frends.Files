...
namespace Frends.FilesRead
{
    public static class FileBatchCommand
    {
        ...
        {
            var results = FilesRead.FindMatchingFiles(input.Directory, input.Pattern);
                ...
                foreach (var entry in fileTransferEntries)
                {
                    ...
                    switch (options.IfTargetFileExists)
                    {
                        case FileExistsAction.Rename:
                            targetFilePath = Frends.FilesRead.FilesRead.GetNonConflictingDestinationFilePath(sourceFilePath, targetFilePath);
            ...
            } catch(Exception)
            {
                ...
                Frends.FilesRead.FilesRead.DeleteExistingFiles(fileResults.Select(x => x.Path));
                throw;
            }
